...
namespace OmegaSudoku
{
    public class SudokuBoard
    {
        ...
        public SudokuBoard(string s) // string ctor - from input format string
        {
        ...
        public bool CheckComplete() // check for correct solution
        {
            if (!this.CheckFilled())
                return false;
            for (int i = 0; i < this._boardSize; i++)
        ...
        private bool CheckFilled() // checking for unfilled cells
        {
            for (int i = 0; i < this._boardSize; i++)
                for (int j = 0; j < this._boardSize; j++)
        ...
        private bool CheckRow(int row) // checks for row conflicts
        {
            int temp = 0;
        ...
        private bool CheckCol(int col) // checks for column conflict
        {
            int temp = 0;
            int[] nums = new int[this._boardSize];
        ...
        private bool CheckSquare(int squareRow, int squareCol) // checks for square conflict
        {
            int temp = 0;
            int[] nums = new int[this._boardSize];
        ...
        public override string ToString() // building a neat board from disgusting strings
        {
            string result = "";
            string seperator = "";
            // building seperator row string (+-------+-------+-------+ for 9*9)
            for (int i = 0; i < this._squareSize; i++)
            {
                seperator += "+";
                for (int j = 0; j < this._squareSize * 2 + 1; j++) 
                    seperator += "-";
            }
            seperator += "+\n";

            for (int i = 0; i < this._squareSize; i++) //building whole board string
            {
                result += seperator;
                for (int j = 0; j < this._squareSize; j++) // building 3 rows(on 9*9)
                {
                    string line = "";
                    for (int k = 0; k < this._squareSize; k++) // building each line
                    {
                        line += "| "; // adding column seperators
                        for (int t = 0; t < this._squareSize; t++) // building 3 digits(in 9*9)
                        {
                            char cell = ' ';
                            ...
                            if (value == 0)
                                ...
                                cell = (char)('A' + value - 10); //A-Z for bigger boards
                            line += cell;
                            line += " "; // seperators between digits
                        }
                    }
                    line += "|\n"; //rightmost edge
                    result += line; // adding current built line to result
                }
            }
            result += seperator; // adding seperator every 3 lines.
            return result;
        }
        public string FormatString() // return the board as string with same format as input string
        {
            string str = "";
